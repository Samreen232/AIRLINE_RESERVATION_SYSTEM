...
$sql = "SELECT * FROM BOOKING WHERE BOOKING_STATUS = 'PROGRESS'";
...
echo $new_fid;
	...
	if(!$result2){
		echo '<script type="text/javascript">alert("FLIGHT ERROR");window.location=\'admin_menu.html\';</script>';
	}
...
$sql = "SELECT * FROM admin where ausername ='$UID' and apassword ='$password'";
$result = $conn->query($sql);

if ($result->num_rows == 1) {
	echo '<script type="text/javascript">alert("Successful login");window.location=\'admin_menu.html\';</script>';
}
else{
		echo '<script type="text/javascript">alert("Wrong Username or Password");window.location=\'admin-login.html\';</script>';
}
$conn->close();
?>
